	...
	<z-paging-swiper>
			...
			<view class="header">
				<view class='top '>
					<view class="top-title font-main-black-16">
						<u-navbar title="创作管理" placeholder bgColor="rgba(0,0,0,0)" :autoBack="true">
						</u-navbar>
					</view>
					<z-tabs inactiveColor="#6F7379" :activeStyle="{'font-size': '16px'}" :bar-style="{'border-radius': '1px',
					'background': 'linear-gradient(270deg, #F53F3F 20%, rgba(245, 63, 63, 0.00) 100%)'}" active-color='#f53f3f'
		...
		<swiper class="swiper" :current="current" @transition="swiperTransition"
			...
			<swiper-item class="swiper-item" v-for="(item, index) in tabList" :key="index">
				...
				<GoodPriceList v-if=' index == 0' :tabIndex="index" :currentIndex="current"></GoodPriceList>
				<NoteList v-if=' index == 1' :tabIndex="index" :currentIndex="current"></NoteList>
			</swiper-item>
		</swiper>
	</z-paging-swiper>
</template>
...
<script>
	...
	import GoodPriceList from './components/goodPriceList.vue'
